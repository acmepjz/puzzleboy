...
bool CLevel::FindPath(CString& ret,int x1,int y1,int x0,int y0) const{
	if(x0<0 || y0<0){
		x0=m_nPlayerX;
		y0=m_nPlayerY;
	}
	...
	CArray<unsigned char> bMoveableData; //0=moveable, 1=blocked, 2=horizontal skip, 3=vertical skip
	bMoveableData.SetSize(m_nWidth*m_nHeight);
	...
	//start BFS :-3
	CArray<int> nodes;
	nodes.SetSize(m_nWidth*m_nHeight);
	...
	while(!queuedNodes.empty()){
		...
		if(currentNode==endNode){
			...
			for(;;){
				...
				//get direction
				ret.AppendChar(x1<x0?'A':y1<y0?'W':x1>x0?'D':'S');
...
bool CLevel::FindPathForBlock(CString& ret,int index,int x1,int y1) const{
	if(index<0 || index>=m_objBlocks.GetSize()) return false;

	const CPushableBlock *targetBlock=(const CPushableBlock*)m_objBlocks[index];
	...
	CArray<unsigned int> bMoveableData;
	bMoveableData.SetSize(m_nWidth*m_nHeight);
	...
	for(int i=0;i<m_objBlocks.GetSize();i++){
		...
		const CPushableBlock *objBlock=(const CPushableBlock*)(m_objBlocks[i]);
		switch(objBlock->m_nType){
		case ROTATE_BLOCK:
			{
				//check if four arms are of equal length
	...
	std::vector<CString> tempSolution;
	{
		CArray<unsigned char> md;
...
#undef EXPAND_1
	...
	std::vector<CString> solutionOfNodes; //(reversed) solution from previous node

	CArray<unsigned int> bTempData;
	bTempData.SetSize(m_nWidth*m_nHeight);
	...
	//create init nodes
	{
		...
		for(int i=0;i<(int)possibleAdjacentX.size();i++){
			//no solution?
			int len=tempSolution[i].GetLength();
		...
		for(int dir=0;dir<4;dir++){
			...
			for(int i=0;i<(int)possibleAdjacentX.size();i++){
				...
				if(idxOfNewNode<0){
					...
					tempSolution[i].AppendChar(nDirections[dir][2]);
					solutionOfNodes.push_back(tempSolution[i]);
	...
	assert(ret.GetLength()==nCurrentBestStep);

	//string reverse
	for(int i=0,m=ret.GetLength();i<m/2;i++){
		int ch=ret[i];
		ret.SetAt(i,ret[m-1-i]);
		ret.SetAt(m-1-i,ch);
	}
...
void CLevel::FindPath2(const CPushableBlock *targetBlock,const BlockBFSNode& src,const int* possibleAdjacentX,const int* possibleAdjacentY,int possibleAdjacentCount,const unsigned int* bMoveableData,unsigned int* bTempData,unsigned int nTimeStamp,CString* ret) const{
	//get current pos
	int x00,y00;
	if(src.pidx<0){
		x00=m_nPlayerX;
...
void CLevel::ValidateRotatingBlock(int index,unsigned int* bTempData,unsigned int nTimeStamp) const{
	const CPushableBlock *objBlock=(const CPushableBlock*)(m_objBlocks[index]);
...
void CLevel::FindPath2_Output(CString& ret,int x0,int y0,int x1,int y1,const unsigned int* bTempData,unsigned int nTimeStamp) const{
	//found the path
	ret.Empty();
...
void CLevel::OptimizeRecord(CString& rec)
{
	if(rec.IsEmpty()) return;

	std::vector<int> prev;
	std::vector<int> steps;
	std::vector<CString> moves;
	...
	//init
	{
		...
		ar.CalculateBlake2s(&checksum,CLevel::ChecksumSize);
		nodeMap[checksum]=0;
	}
	...
	for(int i=0,m=rec.GetLength();i<m;i++){
		...
		case 'A':
		case 'a':
			if(MovePlayer(-1,0,false)) currentMove=_T("A");
			break;
		case 'W':
		case 'w':
			if(MovePlayer(0,-1,false)) currentMove=_T("W");
			break;
		case 'D':
		case 'd':
			if(MovePlayer(1,0,false)) currentMove=_T("D");
			break;
		case 'S':
		case 's':
			if(MovePlayer(0,1,false)) currentMove=_T("S");
			break;
		case '(':
			{
				int x=0,y=0;
				...
				if(SwitchPlayer(x-1,y-1,false)) currentMove=rec.Mid(i0,i-i0+1);
				st=0;
			}
			break;
		default:
		...
		if(currentMove.IsEmpty()) continue;
...
int CLevel::SolveIt(CString& rec,void* userData,int (*callback)(void*,LevelSolverState&)){
	//TODO:
	return 0;
}
