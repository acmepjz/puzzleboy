...
void MySerializer::SetData(const std::vector<unsigned char>& data,int offset){
	m_bIsStoring=false;
	m_bData=data;
	m_nUnsaved=0;
	m_nUnsavedSize=0;
...
void MySerializer::SetFile(u8file* file,bool isStoring,int nChunkSize){
...
void MySerializer::WriteFile(u8file* file) const{
	if(m_bData.size()>0){
...
u16string MySerializer::GetU16String(){
	u16string s;
	int m=GetVUInt32();
	s.reserve(m);
	for(int i=0;i<m;i++){
...
u8string MySerializer::GetU8String(){
	u8string s;
	int m=GetVUInt32();
	s.reserve(m);
	for(int i=0;i<m;i++){
...
void MySerializer::PutU16String(const u16string& s){
	int m=s.size();
	PutVUInt32(m);
	for(int i=0;i<m;i++){
		PutInt16(s[i]);
...
void MySerializer::PutU8String(const u8string& s){
	int m=s.size();
	PutVUInt32(m);
	for(int i=0;i<m;i++){
		PutInt8(s[i]);
...
bool MySerializer::CalculateBlake2s(void* out,int outlen){
	void* lp;
	lp=m_bData.size()>0?(void*)(&(m_bData[0])):(void*)(&lp);

	if(blake2s((uint8_t*)out,lp,NULL,outlen,m_bData.size(),0)<0) return false;
