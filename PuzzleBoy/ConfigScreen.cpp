...
void ConfigScreen::OnDirty(){
	...
	//player name
	m_txtList->NewStringIndex();
	m_txtList->AddString(mainFont,_("Name of Player 1: ")+toUTF8(theApp->m_sPlayerName[0]),
		0,float((m_nListCount++)*32),0,32,1,DrawTextFlags::VCenter);
	m_txtList->NewStringIndex();
	m_txtList->AddString(mainFont,_("Name of Player 2: ")+toUTF8(theApp->m_sPlayerName[1]),
		0,float((m_nListCount++)*32),0,32,1,DrawTextFlags::VCenter);
}
